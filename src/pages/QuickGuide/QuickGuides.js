...
import { deleteQuickGuide, getAllQuickGuides, hideShowQuickGuide } from "../../redux/quickGuideSlice";
...
const QuickGuides = () => {
  ...
  const handleHideShowQuickGuide = (id) => {
    ...
    .catch((error) => {
      ...
      const handleDeleteQuickGuide = ({ id }) => {
        ...
        if (
          !window.confirm("Do You want to permanently delete the selected quick guide?")
        ) {
          return;
        }
  ...
  return (
    <div>
      ...
      {quickGuide?.data && (
        <QuickGuideTable data={quickGuide.data} loading={quickGuide.loading} hideShowQuickGuide={handleHideShowQuickGuide} handleDelete={handleDeleteQuickGuide}/>
      )}
    </div>
  );
};
